...
internal fun parseHtmlToBlocks(html: String): List<ContentBlock> {
    ...
    tagRegex.findAll(html).forEach { m ->
        if (m.range.first > lastIndex) {
            ...
            if (between.isNotBlank()) {
                blocks += ContentBlock.Paragraph(between)
            }
        }
        val tag = m.value
        when {
            ...
            tag.startsWith("<img", ignoreCase = true) -> {
                ...
                if (im != null) {
                    blocks += ContentBlock.Image(im.groupValues[1], im.groupValues.getOrNull(2))
                }
            }
            tag.startsWith("<audio", ignoreCase = true) -> {
                val am = audioRegex.find(tag)
                if (am != null) {
                    blocks += ContentBlock.Audio(am.groupValues[1])
                }
            }
            tag.startsWith("<iframe", ignoreCase = true) -> {
                val ym = youtubeRegex.find(tag)
                if (ym != null) {
                    blocks += ContentBlock.YouTube(ym.groupValues[1])
                }
            }
    ...
    if (lastIndex < html.length) {
        ...
        if (trailing.isNotBlank()) {
            blocks += ContentBlock.Paragraph(trailing)
        }
    }
...
internal fun paginate(blocks: List<ContentBlock>, fontScale: Double, targetCharsPerPage: Int = 1800): List<Page> {
    if (blocks.isEmpty()) {
        return emptyList()
    }
