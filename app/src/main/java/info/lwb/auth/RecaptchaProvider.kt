    ...
    private suspend fun ensureClient(): RecaptchaClient? {
        ...
        return try {
            ...
            if (info.lwb.BuildConfig.DEBUG) {
                runCatching {
                    android.util.Log.d("Recaptcha", "clientFetched siteKeyPrefix=" + siteKey.take(8))
                }
            }
            c
        } catch (e: Exception) {
    ...
    override suspend fun getToken(action: RecaptchaAction): String? = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (info.lwb.BuildConfig.DEBUG) {
                ...
                if (ex != null) {
                    runCatching {
                        android.util.Log.w("Recaptcha", "token failure: ${ex::class.java.simpleName} msg=${ex.message}")
                    }
                }
                runCatching {
                    android.util.Log.d(
                        "Recaptcha",
                        "tokenResult null=${t == null} length=${t?.length} action=$action",
                    )
                }
            }
            t
