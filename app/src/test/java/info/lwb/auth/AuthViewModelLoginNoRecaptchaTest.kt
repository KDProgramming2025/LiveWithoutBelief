...
class AuthViewModelLoginNoRecaptchaTest {
    ...
    fun passwordLogin_doesNotRequireRecaptcha() = runTest {
        ...
        val facade = object: AuthFacade {
            override suspend fun oneTapSignIn(activity: android.app.Activity) = Result.failure<AuthUser>(UnsupportedOperationException())
            override fun currentUser(): AuthUser? = null
            override suspend fun signOut() { }
            override suspend fun refreshIdToken(forceRefresh: Boolean) = Result.failure<String>(UnsupportedOperationException())
            override suspend fun register(username: String, password: String, recaptchaToken: String?) = Result.failure<AuthUser>(UnsupportedOperationException())
