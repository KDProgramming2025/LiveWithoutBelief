...
class InMemorySecureStorage : SecureStorage {
    ...
    override fun putProfile(name: String?, email: String?, avatar: String?) {
        this.name = name; this.email = email; this.avatar = avatar
    }
...
interface CredentialCall {
    suspend fun get(activity: Activity, request: androidx.credentials.GetCredentialRequest): androidx.credentials.GetCredentialResponse
}
...
class CredentialManagerOneTapProvider(private val call: CredentialCall) {
    suspend fun getIdToken(activity: Activity): String? = runCatching {
        val req = androidx.credentials.GetCredentialRequest(listOf())
